...
if uploaded_file is not None or 'SAP_CTPM' in st.session_state:
    ...
    if toggle_op_eqp:
        ...
        valores_presentes_op_eqp_colar = SAP_EQP_filt['Equipamento'].isin(op_eqp_colar)
        op_eqp_colar = SAP_EQP_filt.loc[valores_presentes_op_eqp_colar, 'Equipamento'].tolist()
    ...
    @st.experimental_dialog("📝 Descrição dos Equipamentos Selecionados")
    def processar_opcoes_equipamento(SAP_EQP_filt, op_eqp):
        if not isinstance(op_eqp, str):
            for option in op_eqp:
                index_procurado = SAP_EQP_filt[SAP_EQP_filt['Equipamento'] == option].index[0]
                st.write(option, ":", SAP_EQP_filt['Denominação do objeto técnico'][index_procurado])
        else:
            index_procurado = SAP_EQP_filt[SAP_EQP_filt['Equipamento'] == op_eqp].index[0]
            st.write(SAP_EQP_filt['Denominação do objeto técnico'][index_procurado])

    @st.experimental_dialog("🔎 Código de Equipamento por Nome")
    def pesquisar_equipamento_por_nome(SAP_EQP_filt):
        pesquisar_eqp = st.text_input("Pesquisar Equipamento:")
        pesquisar_eqp = pesquisar_eqp.upper()
        PROC_EQP = SAP_EQP_filt[
            SAP_EQP_filt['Denominação do objeto técnico'].str.contains(str(pesquisar_eqp))
    ...
    colx, coly = col1.columns([2, 2])
    if coly.button("🔎 Pesquisar Códigos", use_container_width=True):
        pesquisar_equipamento_por_nome(SAP_EQP_filt)
    if colx.button("📝 Equip Selecionados", use_container_width=True):
        processar_opcoes_equipamento(SAP_EQP_filt, op_eqp)
    ...
    op_oppadrao = col1.text_input(label='Operação Padrão',
                                  max_chars=40, key='OP_OPPADRAO', placeholder="Digite aqui",
                                  help='Escreva o texto descritivo principal da operação em 40 caracteres. Abrevie se necessário.')
    op_optextolongo = col1.text_area(label='Texto Longo', key='OP_OPTEXTOLONGO', placeholder="Digite aqui",
                                     help='Escreva um texto detalhado da operação ou um que contenha detalhes adicionais não presentes no texto principal.')
    ...
    op_tipoativ = col1.selectbox(
        ...
        options=['REVISÃO', 'INSPEÇÃO', 'LUBRIFICAÇÃO', 'TROCA', 'LIMPEZA', 'REAPERTO', 'TESTE', 'AJUSTE',
                 'CALIBRAÇÃO'], key='OP_TIPOATIV'
    )

    #   Selecionar periodicidade
    op_periodicidade = col1.selectbox(label='Periodicidade',
                                      options=['1D', '7D', '15D', '1M', '2M', '3M', '4M', '5M', '6M', '7M', '8M', '9M',
                                               '10M', '11M'
    ...
    op_erota = col1.radio(
        label='Rota?',
        options=['SIM', 'NÃO', 'PERSONALIZADO'], key='OP_EROTA',
        help='Sendo rota, no título da task list será o local de instalação dos equipamentos. Para rota personalizada, insira PERSONALIZADO'
    )
    if op_erota == 'NÃO' or op_erota == 'PERSONALIZADO':
        ...
        op_trecho2tasklist = col1.text_input(label=texto_rota_nao if op_erota == 'NÃO' else texto_rota_personalizado,
                                             ...
                                             max_chars=20 - len(op_periodicidade) if op_estadomaq == 1 else 25 - len(
                                                 op_periodicidade),
                                             # if op_erota == 'NÃO' and op_estadomaq == 0 else '',
    ...
    op_espec = col1.radio(
        ...
        help='Esse parâmetro estará contido no título da task list. Ele representa a especialidade para a task list como um todo.'
    )
    ...
    if adc_op or adc_op_mat:  # Ao clicar botão de adicionar operação
        ...
        for i in range(len(op_eqp_)):
            ...
            task_list_parcial_ = (op_tipoativ[0:4] + ' ' + op_espec[
                ...
                '  ', ' ') if op_erota == 'NÃO' or op_erota == 'PERSONALIZADO' else (
                        op_tipoativ[0:4] + ' ' + op_espec[0:3] + ' ' + op_estadomaq_ + ' ' + op_periodicidade).replace(
                '  ', ' ')
            lista_add_OP_PADRAO.append(task_list_parcial_)
            lista_add_OP_PADRAO = lista_add_OP_PADRAO + [op_oppadrao, op_optextolongo, op_ctpm, op_duracao, op_operad,
                                                         op_trabtot]
            ...
            if len(task_list_completa_) > 40:
                task_list_completa_ = task_list_completa_.replace('.', '').replace(' N0', ' ').replace(' DE ',
                                                                                                       ' ').replace(
                    ' DA ', ' ').replace(' DO ', ' ').replace('  ', ' ')
            if len(task_list_completa_) > 40:
                task_list_completa_ = task_list_completa_.replace(' RESFRIAMENTO ', ' RESF ').replace(' RESFRIADOR ',
                                                                                                      ' RESF ').replace(
                    ' REFRIGERACAO ', ' REFRIG ').replace(' EMBALAGEM ', ' EMB ').replace(' EMBALADORA ',
                                                                                          ' EMB ').replace(
                    ' EMBALADOR ', ' EMB ').replace(' ENCAIXOTAMENTO ', ' ENCX ').replace(' ENCAIXOTADOR ',
                                                                                          ' ENCX ').replace(
                    ' ENCAIXOTADORA ', ' ENCX ').replace(' EMPILHAMENTO ', ' EMPI ').replace(' APLICACAO ',
                                                                                             ' APL ').replace(
                    ' PREPARO ', ' PREP ').replace(' PREPARACAO ', ' PREP ').replace(' BISCOITO ', ' BISC ').replace(
                    ' WAFER ', ' WAF ').replace(' TRANSPORTE ', ' TRANSP ').replace(' TRANSFERENCIA ',
                                                                                    ' TRANSF ').replace(' ARMAZENAGEM ',
                                                                                                        ' ARMZ ').replace(
                    ' ALIMENTACAO ', ' ALIM ').replace(' SEPARACAO ', ' SEP ').replace(' RECEBIMENTO ',
                                                                                       ' RECEB ').replace(' MARGARINA ',
                                                                                                          ' MARG ').replace(
                    ' DISTRIBUICAO ', ' DISTRIB ').replace(' GERACAO ', ' GER ').replace(' COMPRIMIDO ',
                                                                                         ' COMP ').replace('  ', ' ')
    ...
    OP_PADRAO.sort_values(by=['Planta*', 'TASK LIST ORIGINAL', 'TEXTO TIPO EQUIPAMENTO'],
    ...
    OP_PADRAO.drop(OP_PADRAO[(OP_PADRAO['TRECHO 02_TASK LIST'] == '') & (OP_PADRAO['ROTA?'] == 'NÃO')].index,
                   inplace=True)
    OP_PADRAO.drop(OP_PADRAO[(OP_PADRAO['QTD_MATERIAL'] == 0) & (OP_PADRAO['TEM MATERIAL?'] == 'SIM')].index,
                   inplace=True)
    OP_PADRAO.reset_index(drop=True, inplace=True)
    ...
    if not OP_PADRAO.empty:  # Caso o df esteja vazio
        OP_PADRAO = col2.data_editor(
            OP_PADRAO,
            column_order=[
                'TASK LIST ORIGINAL', 'OPERACAO PADRAO', 'TEXTO DESCRITIVO', 'TEXTO TIPO EQUIPAMENTO',
                'TIPO EQUIPAMENTO',
    ...
    def reset():
        if not OP_PADRAO.empty:
            ...
            st.session_state.OP_QTDMATERIAL = OP_PADRAO["QTD_MATERIAL"][index_remov_edit] if not isinstance(
                OP_PADRAO["QTD_MATERIAL"][index_remov_edit], str) else None
    ...
    excluir_linha = col4.button('Selecionar linha', disabled=True if index_remov_edit == None else False,
                                on_click=reset)
    ...
    for i in range(len(OP_PADRAO['TEXTO TIPO EQUIPAMENTO'])):
        ...
        if ' FUNC ' not in OP_PADRAO['TASK LIST ORIGINAL'][i] and OP_PADRAO['TASK LIST ORIGINAL'][i].split(' ')[2][
            -1] == 'D':
        ...
        if isinstance(OP_PADRAO['OPERACAO PADRAO'][i], str):
            if not any(name in OP_PADRAO['OPERACAO PADRAO'][i] for name in
                ...
                OP_PADRAO_SHOW_AJUS.loc[len(OP_PADRAO_SHOW_AJUS)] = [i,
                                                                     "VERIFICAR SE EM 'OPERAÇÃO PADRÃO' É ESPECIFICADO TIPO DE ATIVIDADE DESEMPENHADA (INSP, REVI, ETC)"]
